package com.yudao.lianying.v1.field.api;
 ...
 * @since 2020-12-12
 */
@Api(tags = "产品线和具体型号关系表")
@RequestMapping(value = "/fieldApplicableRelation", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
public interface FieldApplicableRelationServiceApi {

    @ApiOperation(value = "保存单条-自动生成")
    @CrossOrigin(allowCredentials = "true", allowedHeaders = "*", methods = {RequestMethod.POST}, origins = "*")
    @RequestMapping(value = "/saveAuto", method = {RequestMethod.POST})
    Result saveAuto(FieldApplicableRelationVO entityVO);

    @ApiOperation(value = "根据id删除，允许传入多个id-自动生成")
    @CrossOrigin(allowCredentials = "true", allowedHeaders = "*", methods = {RequestMethod.POST}, origins = "*")
    @RequestMapping(value = "/removeByIdAuto", method = {RequestMethod.POST})
    @ApiImplicitParams({
            @ApiImplicitParam(name = "ids", value = "产品线和具体型号关系表主键，多个id以逗号分隔", required = true),
    })
    ...
    Result modifyAllColumnInObjectAuto(FieldApplicableRelationModifyVO1 modifyVO1);
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(name = "id", value = "产品线和具体型号关系表主键", required = true),
    })
