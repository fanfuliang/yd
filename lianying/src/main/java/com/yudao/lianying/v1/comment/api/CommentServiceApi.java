package com.yudao.lianying.v1.comment.api;
...
public interface CommentServiceApi {
    ...
    Result save(CommentVO entityVO);

    @ApiOperation(value = "批量纠错")
    @CrossOrigin(allowCredentials = "true", allowedHeaders = "*", methods = {RequestMethod.POST}, origins = "*")
    @RequestMapping(value = "/saveBatch", method = {RequestMethod.POST})
    Result saveBatch(CommentBathVO entityVO);

    @ApiOperation(value = "根据id删除，允许传入多个id-自动生成")
    @CrossOrigin(allowCredentials = "true", allowedHeaders = "*", methods = {RequestMethod.POST}, origins = "*")
    @RequestMapping(value = "/removeByIdAuto", method = {RequestMethod.POST})
    @ApiImplicitParams({
            @ApiImplicitParam(name = "ids", value = "纠错表主键，多个id以逗号分隔", required = true),
    })
    ...
    Result modifyAllColumnInObjectAuto(CommentModifyVO1 modifyVO1);
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(name = "id", value = "纠错表主键", required = true),
    })
    ...
    @ApiOperation(value = "获取entry的历史纠错")
    @CrossOrigin(allowCredentials = "true", allowedHeaders = "*", methods = {RequestMethod.GET}, origins = "*")
    @RequestMapping(value = "/getCommentByEntry", method = {RequestMethod.GET})
    @ApiImplicitParams({
            @ApiImplicitParam(name = "entryId", value = "qterm里的entry_id", required = true),
    })
    Result getCommentByEntry(@ParameterValidtion(notEmpty = true, name = "entryId") String entryId);
