package com.yudao.lianying.v1.whitelist.api;
...
public interface WhitelistServiceApi {
    ...
    Result modifyAllColumnInObjectAuto(WhitelistModifyVO1 modifyVO1);
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(name = "id", value = "白名单主键", required = true),
    })
    ...
    @ApiOperation(value = "验证是否可以检查")
    @CrossOrigin(allowCredentials = "true", allowedHeaders = "*", methods = {RequestMethod.POST}, origins = "*")
    @RequestMapping(value = "/canCheck", method = {RequestMethod.POST})
    @ApiImplicitParams({
            @ApiImplicitParam(name = "loginName", value = "登录名", required = true),
    })
    Result canCheck(@ParameterValidtion(notEmpty = true, name = "loginName") String loginName);
}
