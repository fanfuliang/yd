 ...
 * @since 2020-12-08
 */
@Api(tags = "术语_entry_语言")
@RequestMapping(value = "/entryLang", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
public interface EntryLangServiceApi {

    @ApiOperation(value = "保存单条-自动生成")
    @CrossOrigin(allowCredentials = "true", allowedHeaders = "*", methods = {RequestMethod.POST}, origins = "*")
    @RequestMapping(value = "/saveAuto", method = {RequestMethod.POST})
    Result saveAuto(EntryLangVO entityVO);
    ...
    Result modifyAllObjectFeildAuto(EntryLangModifyVO1 modifyVO1);
    ...
    @ApiOperation(value = "获取用户最常用的三个语言")
    @CrossOrigin(allowCredentials = "true", allowedHeaders = "*", methods = {RequestMethod.GET}, origins = "*")
    @RequestMapping(value = "/getCommonLang", method = {RequestMethod.GET})
    @ApiImplicitParams({
            @ApiImplicitParam(name = "account", value = "用户", required = true),
    })
    Result getCommonLang(@ParameterValidtion(notEmpty = true, name = "account") String account);
}
