 ...
 */
@Api(tags = "术语点击历史")
@RequestMapping(value = "/clickHistory", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
public interface ClickHistoryServiceApi {

    @ApiOperation(value = "保存单条-自动生成")
    @CrossOrigin(allowCredentials = "true", allowedHeaders = "*", methods = {RequestMethod.POST}, origins = "*")
    @RequestMapping(value = "/saveAuto", method = {RequestMethod.POST})
    Result saveAuto(ClickHistoryVO entityVO);

    @ApiOperation(value = "根据id删除，允许传入多个id-自动生成")
    @CrossOrigin(allowCredentials = "true", allowedHeaders = "*", methods = {RequestMethod.POST}, origins = "*")
    @RequestMapping(value = "/removeByIdAuto", method = {RequestMethod.POST})
    @ApiImplicitParams({
            @ApiImplicitParam(name = "ids", value = "术语点击历史主键，多个id以逗号分隔", required = true),
    })
    ...
    Result modifyAllColumnInObjectAuto(ClickHistoryModifyVO1 modifyVO1);
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(name = "id", value = "术语点击历史主键", required = true),
    })
    ...
    @CrossOrigin(allowCredentials = "true", allowedHeaders = "*", methods = {RequestMethod.GET}, origins = "*")
    @RequestMapping(value = "/hotTerm", method = {RequestMethod.GET})
    Result hotTerm();
