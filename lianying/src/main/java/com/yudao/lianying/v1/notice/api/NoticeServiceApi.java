package com.yudao.lianying.v1.notice.api;


import com.yudao.common.yudaocommon.annotation.ParameterValidtion;
import com.yudao.common.yudaocommon.model.Result;
import com.yudao.lianying.v1.notice.vo.NoticeVO;
import com.yudao.lianying.v1.notice.vo.NoticeModifyVO1;
...
public interface NoticeServiceApi {
    ...
    Result saveAuto(NoticeVO entityVO);
    ...
    Result modifyAllColumnInObjectAuto(NoticeModifyVO1 modifyVO1);
    ...
    @ApiOperation(value = "查询未读通知的数量")
    @CrossOrigin(allowCredentials = "true", allowedHeaders = "*", methods = {RequestMethod.GET}, origins = "*")
    @RequestMapping(value = "/myUnreadNoticeNum", method = {RequestMethod.GET})
    @ApiImplicitParams({
            @ApiImplicitParam(name = "account", value = "用户名", required = true),
    })
    Result myUnreadNoticeNum(@ParameterValidtion(notEmpty = true, name = "account") String account);
}
