    ...
    public String query(String outTradeNo){
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                return result.getResponse().getTradeNo();
            case FAILED:
...
//        model.setOutRequestNo(UUID.randomUUID().toString());
    ...
    public boolean rsaCheckV2(HttpServletRequest request) {
        Map<String, String> params = getAlipayParams(request);
        try {
            return AlipaySignature.rsaCheckV2(params, aliPayProperties.getAlipayPublicKey(), aliPayProperties.getCharset(), aliPayProperties.getSignType());
        } catch (AlipayApiException e) {
            log.debug("verify sigin error, exception is:{}", e);
            return false;
        }
    ...
    /**
     * 获取支付宝POST过来反馈信息，参考https://docs.open.alipay.com/54/106370
     * @param request
     * @return
     */
